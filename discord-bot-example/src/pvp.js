  ...
  if (!fs.existsSync(file)) {
    throw new Error(
      [
        ...
        'Point SQLITE_FILE at the plugin database, usually plugins/UltimateDonutSmp2/data/data.db.'
      ].join('\n')
    );
  }
...
async function createMongoPvpRepository() {
  ...
  const database = client.db(process.env.MONGODB_DATABASE || 'ultimatedonutsmp2');
