...
const defaultSqliteFile = '../plugins/UltimateDonutSmp2/data/data.db';
  ...
  if (!fs.existsSync(databasePath)) {
    throw new Error(
      [
        ...
        'Point SQLITE_FILE to the real UltimateDonutSmp2 database, usually plugins/UltimateDonutSmp2/data/data.db, or copy that data.db file into this bot project.'
      ].join('\n')
    );
  }
...
async function createMysqlRepository() {
  const pool = mysql.createPool({
    ...
    database: process.env.MYSQL_DATABASE || 'ultimatedonutsmp2',
...
async function createMongoRepository() {
  ...
  const databaseName = process.env.MONGODB_DATABASE || 'ultimatedonutsmp2';
