...
public class CrateEditorMenu extends BaseMenu {
    ...
    public CrateEditorMenu(UltimateDonutSmp2 plugin, String crateId) {
    ...
    private ItemStack createEditorItem(CrateManager.CrateReward reward) {
        ...
        if (reward.grant().serializedItemData() != null && !reward.grant().serializedItemData().isBlank()) {
            try {
                item = com.bx.ultimateDonutSmp2.utils.ItemSerializationUtils.deserialize(reward.grant().serializedItemData());
    ...
    private static int resolveSize(UltimateDonutSmp2 plugin, String crateId) {
