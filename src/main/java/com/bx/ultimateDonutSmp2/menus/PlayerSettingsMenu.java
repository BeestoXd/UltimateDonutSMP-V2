...
public final class PlayerSettingsMenu extends BaseMenu {
    ...
    public PlayerSettingsMenu(UltimateDonutSmp2 plugin) {
        super(
                plugin,
    ...
    public void handleClick(int slot, Player player) {
        ...
        switch (key) {
            ...
            case "NIGHT_VISION" -> {
                boolean enabled = com.bx.ultimateDonutSmp2.utils.NightVisionUtils.toggle(plugin, player);
        ...
        return switch (key) {
            ...
            case "DISABLE_MOB_SPAWN" -> {
                ...
                if (disabled && data.getMobSpawnDisabledUntil() > 0) {
                    ...
                    if (remainingSecs > 0) {
                        yield new ButtonState("&aEnabled &7(" + com.bx.ultimateDonutSmp2.utils.NumberUtils.formatTime(remainingSecs) + " left)", true);
                    }
                }
            ...
            case "NIGHT_VISION" -> state(com.bx.ultimateDonutSmp2.utils.NightVisionUtils.isEnabled(plugin, player));
    ...
    private ButtonState quickBuyState(Player player) {
        if (!PermissionUtils.has(player, "ultimatedonutsmp2.auctionhouse.fastbuy")
    ...
    private ButtonState quickSellState(Player player) {
        if (!PermissionUtils.has(player, "ultimatedonutsmp2.auctionhouse.fastsell")
    ...
    private String formatThreeChoice(ThreeChoice choice) {
        return switch (choice) {
            case OFF -> "&cOff";
            case ANYONE -> "&aAnyone";
            case FRIENDS_FOLLOWED -> "&eFriends/Followed";
        };
    }
