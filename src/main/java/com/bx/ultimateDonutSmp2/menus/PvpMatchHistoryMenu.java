...
import com.bx.ultimateDonutSmp2.managers.PvpMatchManager;
import com.bx.ultimateDonutSmp2.models.PvpMatch;
...
public class PvpMatchHistoryMenu extends BaseMenu {
    ...
    public PvpMatchHistoryMenu(UltimateDonutSmp2 plugin, UUID targetUuid) {
    ...
    private ItemStack createMatchItem(PvpMatch match, int number) {
        ...
        String firstName = plugin.getHideManager() != null
                ? plugin.getHideManager().publicName(match.getFirstUuid(), matches.resolveName(match.getFirstUuid()))
                : matches.resolveName(match.getFirstUuid());
        String secondName = plugin.getHideManager() != null
                ? plugin.getHideManager().publicName(match.getSecondUuid(), matches.resolveName(match.getSecondUuid()))
                : matches.resolveName(match.getSecondUuid());
        lore.add("");
        lore.add("&f" + firstName + " &7vs &f" + secondName);
        lore.add("");
    ...
    private void appendPlayerLines(List<String> lore, PvpMatch match, UUID uuid) {
        ...
        String name = plugin.getHideManager() != null
                ? plugin.getHideManager().publicName(uuid, matches.resolveName(uuid))
                : matches.resolveName(uuid);
        lore.add("&a" + name);
    ...
    private static String configuredTitle(UltimateDonutSmp2 plugin, UUID targetUuid) {
