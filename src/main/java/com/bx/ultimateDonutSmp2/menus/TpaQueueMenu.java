...
public class TpaQueueMenu extends BaseMenu {
    ...
    public TpaQueueMenu(UltimateDonutSmp2 plugin, boolean tpaHere) {
        this(plugin, tpaHere, 0);
    }

    public TpaQueueMenu(UltimateDonutSmp2 plugin, boolean tpaHere, int page) {
    ...
    private String resolveName(UUID uuid) {
        ...
        if (online != null) {
            fallback = online.getName();
        } else {
            OfflinePlayer offline = Bukkit.getOfflinePlayer(uuid);
            fallback = offline.getName();
        }
        if (fallback == null || fallback.isBlank()) {
            fallback = uuid.toString().substring(0, 8);
        }
        return plugin.getHideManager() != null ? plugin.getHideManager().publicName(uuid, fallback) : fallback;
    }
    ...
    private static String configuredTitle(UltimateDonutSmp2 plugin, boolean tpaHere) {
