...
public class TeamKickConfirmMenu extends BaseMenu {
    ...
    public TeamKickConfirmMenu(UltimateDonutSmp2 plugin, UUID targetUuid, int returnPage, TeamMenu.SortMode returnSortMode, String returnSearchQuery) {
    ...
    private String resolveTargetName() {
        String fallback = Bukkit.getOfflinePlayer(targetUuid).getName();
        if (fallback == null) {
            String stored = plugin.getDatabaseManager().getLastKnownUsername(targetUuid);
            fallback = stored != null ? stored : "player";
        }
        return plugin.getHideManager() != null
                ? plugin.getHideManager().publicName(targetUuid, fallback)
                : fallback;
    }
    ...
    private static String configuredTitle(UltimateDonutSmp2 plugin, UUID targetUuid) {
        ...
        if (plugin.getHideManager() != null) {
            name = plugin.getHideManager().publicName(targetUuid, name);
        }
        return plugin.getConfigManager().getMenus()
