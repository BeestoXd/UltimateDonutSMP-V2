...
public class ShopMenu extends BaseMenu {
    ...
    public ShopMenu(UltimateDonutSmp2 plugin) {
        this(plugin, null, 0, false);
    }

    public ShopMenu(UltimateDonutSmp2 plugin, String menuSection) {
        this(plugin, menuSection, 0, false);
    }

    public ShopMenu(UltimateDonutSmp2 plugin, String menuSection, int page) {
        this(plugin, menuSection, page, false);
    }

    public ShopMenu(UltimateDonutSmp2 plugin, boolean favoritesMenu, int page) {
        this(plugin, null, page, favoritesMenu);
    }

    private ShopMenu(UltimateDonutSmp2 plugin, String menuSection, int page, boolean favoritesMenu) {
        super(
                plugin,
    ...
    private void buildFavoritesControl(Player player) {
        ...
        set(config().getInt(path + ".SLOT", 26), ItemUtils.createItem(
                ...
                replace(config().getString(path + ".NAME", "&eFavorite items"), "{count}", String.valueOf(count)),
    ...
    private String replaceShopGuiPlaceholders(
        ...
        String favoriteLine = config().getString(
                ...
                favorite ? "&e★ Favorite" : "&7☆ Not favorited"
        );
        String favoriteAction = favoritesEnabled()
                ? config().getString("SHOP-GUI.ITEM.FAVORITE-ACTION", "&eShift-click to toggle favorite")
                : "";
    ...
    private static String resolveTitle(UltimateDonutSmp2 plugin, String menuSection, boolean favoritesMenu) {
        if (plugin == null || plugin.getConfigManager() == null || plugin.getConfigManager().getShop() == null) {
            return favoritesMenu ? "&8Favorite shop items" : (menuSection == null ? "&8Shop" : "&8Shop");
        }
    ...
    private static int resolveSize(UltimateDonutSmp2 plugin, String menuSection, boolean favoritesMenu) {
        if (plugin == null || plugin.getConfigManager() == null || plugin.getConfigManager().getShop() == null) {
            return favoritesMenu ? 54 : 27;
        }
        FileConfiguration config = plugin.getConfigManager().getShop();
    ...
    public boolean isSameScreen(BaseMenu other) {
        if (!(other instanceof ShopMenu otherShop)) {
            return false;
        }
        if (this.mainMenu != otherShop.mainMenu) {
            return false;
        }
        if (this.favoritesMenu != otherShop.favoritesMenu) {
            return false;
        }
        return java.util.Objects.equals(this.menuSection, otherShop.menuSection);
    }
}
