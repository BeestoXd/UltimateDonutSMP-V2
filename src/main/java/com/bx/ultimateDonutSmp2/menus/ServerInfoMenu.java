...
public class ServerInfoMenu extends BaseMenu {
    ...
    public ServerInfoMenu(UltimateDonutSmp2 plugin) {
        this(plugin, 1);
    }

    public ServerInfoMenu(UltimateDonutSmp2 plugin, int page) {
        this(plugin, page, loadPages(plugin));
    }

    private ServerInfoMenu(UltimateDonutSmp2 plugin, int requestedPage, List<PageDefinition> loadedPages) {
    ...
    public void build(Player player) {
        ...
        if (plugin.getRtpManager() != null) {
            plugin.getRtpManager().prefetchMenuDestinations();
        }
    }
    ...
    public void handleClick(int slot, Player player) {
        ...
        if (action.type() == ActionType.COMMAND) {
            ...
            if (dispatchRtpCommandNow(player, action.command())) {
                return;
            }
    ...
    /**
     * RTP from this menu used to close, wait a tick, then run {@code /rtp}, which started a
     * live search on an empty cache. Closing through {@link BaseMenu#close} and queueing the
     * teleport on this tick is what makes the button feel like a normal teleport.
     */
    private boolean dispatchRtpCommandNow(Player player, String command) {
        String selector = rtpCommandSelector(command);
        if (selector == null || plugin.getRtpManager() == null) {
            return false;
        }
        close(player);
        if (selector.isEmpty()) {
            ...
            plugin.getRtpManager().queueCommandTeleport(player, selector);
        }
        return true;
    }

    static String rtpCommandSelector(String command) {
        ...
        if (!"rtp".equalsIgnoreCase(label)) {
            return null;
        }
        if (space < 0) {
            return "";
        }
        String selector = normalized.substring(space + 1).trim();
    ...
    private static List<PageDefinition> loadPages(UltimateDonutSmp2 plugin) {
    ...
    private static List<ButtonDefinition> loadButtons(UltimateDonutSmp2 plugin, ConfigurationSection buttonsSection, String pageKey) {
    ...
    private static ButtonAction resolveAction(UltimateDonutSmp2 plugin, String key, ConfigurationSection buttonSection) {
        ...
        return switch (key.toUpperCase(Locale.ROOT)) {
            ...
            case "ECONOMY" -> ButtonAction.info(List.of(
                    "&7Earn " + plugin.getCurrencyManager().plural(com.bx.ultimateDonutSmp2.managers.CurrencyManager.CurrencyType.MONEY)
    ...
    private static String configuredTitle(UltimateDonutSmp2 plugin, int requestedPage, List<PageDefinition> loadedPages) {
        if (!loadedPages.isEmpty()) {
    ...
    private static int configuredSize(UltimateDonutSmp2 plugin, int requestedPage, List<PageDefinition> loadedPages) {
        if (!loadedPages.isEmpty()) {
            int safePage = Math.max(1, Math.min(requestedPage, loadedPages.size()));
    ...
    private static PageDefinition createDefaultGettingStartedPage(UltimateDonutSmp2 plugin) {
        String moneyPlural = plugin.getCurrencyManager().plural(com.bx.ultimateDonutSmp2.managers.CurrencyManager.CurrencyType.MONEY);
    ...
    private static PageDefinition createDefaultCommandsPage(UltimateDonutSmp2 plugin) {
        String moneyPlural = plugin.getCurrencyManager().plural(com.bx.ultimateDonutSmp2.managers.CurrencyManager.CurrencyType.MONEY);
        return new PageDefinition(
                "3",
                "&8Useful commands",
