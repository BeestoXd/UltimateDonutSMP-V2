...
import com.bx.ultimateDonutSmp2.menus.SpawnMenu;
...
public class SpawnCommand implements CommandExecutor {
    ...
    public SpawnCommand(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0 && (args[0].equalsIgnoreCase("set") || args[0].equalsIgnoreCase("setspawn") || args[0].equalsIgnoreCase("setup"))) {
            if (!PermissionUtils.has(player, "ultimatedonutsmp2.command.setspawn") && !PermissionUtils.has(player, "ultimatedonutsmp2.admin.setup")) {
