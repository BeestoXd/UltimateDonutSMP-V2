...
public class AddMoneyCommand implements CommandExecutor {
    ...
    private static final String PERMISSION = "ultimatedonutsmp2.admin.addmoney";

    private final UltimateDonutSmp2 plugin;

    public AddMoneyCommand(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        var result = plugin.getEconomyManager().deposit(account, amount, com.bx.ultimateDonutSmp2.models.EconomyReason.ADMIN_ADD);
        if (!result.success()) {
            sender.sendMessage(ColorUtils.toComponent(plugin.getConfigManager().getMessage("BALANCE.ADMIN.PLAYER-NOT-FOUND")));
            return true;
        }
