...
public class HealCommand implements CommandExecutor {
    ...
    private static final String PERMISSION = "ultimatedonutsmp2.staff.heal";

    private final UltimateDonutSmp2 plugin;

    public HealCommand(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        sender.sendMessage(ColorUtils.toComponent(
                plugin.getConfigManager().getMessageOrDefault("HEAL.OTHER", "&7You healed &e%player%", "%player%", target.getName())
        ));
        target.sendMessage(ColorUtils.toComponent(
                plugin.getConfigManager().getMessageOrDefault("HEAL.NOTIFY", "&e%sender% &7restored your health", "%sender%", senderName)
        ));
        return true;
    }
