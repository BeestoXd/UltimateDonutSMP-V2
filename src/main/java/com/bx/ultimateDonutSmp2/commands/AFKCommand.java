...
import com.bx.ultimateDonutSmp2.menus.AfkMenu;
...
public class AFKCommand implements CommandExecutor {
    ...
    public AFKCommand(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0 && (args[0].equalsIgnoreCase("set") || args[0].equalsIgnoreCase("setafk") || args[0].equalsIgnoreCase("setup"))) {
            if (!PermissionUtils.has(player, "ultimatedonutsmp2.command.setafk") && !PermissionUtils.has(player, "ultimatedonutsmp2.admin.setup")) {
