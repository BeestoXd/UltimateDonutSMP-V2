...
public class TeleportCommand implements CommandExecutor {
    ...
    private static final String PERMISSION = "ultimatedonutsmp2.staff.teleport";

    private final UltimateDonutSmp2 plugin;

    public TeleportCommand(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
                ...
                plugin.getSpigotScheduler().runEntity(player, () -> {
                    if (Boolean.TRUE.equals(success) && player.isOnline()) {
                        player.sendMessage(ColorUtils.toComponent(
                                message("TELEPORT.TO_PLAYER", "&eTeleported &7to %player%")
    ...
    private void teleportHere(Player player, String input) {
        ...
        plugin.getSpigotScheduler().teleport(target, destination).thenAccept(success -> {
            ...
            plugin.getSpigotScheduler().runEntity(player, () -> {
                if (player.isOnline()) {
                    player.sendMessage(ColorUtils.toComponent(
                            message("TELEPORT.HERE", "&eTeleported &7%player% to your location")
                                    .replace("%player%", targetName),
                            player
                    ));
                }
            ...
            plugin.getSpigotScheduler().runEntity(target, () -> {
                if (target.isOnline()) {
                    target.sendMessage(ColorUtils.toComponent(
                            message("TELEPORT.HERE_TARGET", "&eYou were teleported to &7%sender%")
    ...
    private void teleportAll(Player player) {
            ...
            plugin.getSpigotScheduler().teleport(target, destination).thenAccept(success ->
                    plugin.getSpigotScheduler().runEntity(target, () -> {
                        if (Boolean.TRUE.equals(success) && target.isOnline()) {
                            target.sendMessage(ColorUtils.toComponent(
                                    message("TELEPORT.ALL_TARGET", "&eYou were teleported to &7%sender%")
                                            .replace("%sender%", senderName),
                                    target
                            ));
                        }
        ...
        player.sendMessage(ColorUtils.toComponent(
                message("TELEPORT.ALL", "&eTeleported &7all players to your location"),
                player
        ));
    }
    ...
    private void teleportTop(Player player) {
        ...
        plugin.getSpigotScheduler().teleport(player, current).thenAccept(success ->
                plugin.getSpigotScheduler().runEntity(player, () -> {
                    if (Boolean.TRUE.equals(success) && player.isOnline()) {
                        player.sendMessage(ColorUtils.toComponent(
                                message("TELEPORT.TOP", "&eTeleported &7to the highest position"),
                                player
                        ));
                    }
                }));
    ...
    private void teleportToPosition(Player player, String[] args) {
        ...
        plugin.getSpigotScheduler().teleport(player, destination).thenAccept(success ->
                plugin.getSpigotScheduler().runEntity(player, () -> {
                    if (Boolean.TRUE.equals(success) && player.isOnline()) {
                        player.sendMessage(ColorUtils.toComponent(
                                message("TELEPORT.POSITION", "&7Teleported to: &e%x%,%y%,%z% &7(%world%)")
