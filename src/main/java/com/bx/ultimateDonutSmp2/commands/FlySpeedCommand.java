...
public class FlySpeedCommand implements CommandExecutor {
    ...
    private static final String PERMISSION = "ultimatedonutsmp2.command.flyspeed";
    private static final String STAFF_PERMISSION = "ultimatedonutsmp2.staff.flyspeed";
    private static final String FLY_STAFF_PERMISSION = "ultimatedonutsmp2.staff.fly";

    private final UltimateDonutSmp2 plugin;

    public FlySpeedCommand(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
        ...
        if (args.length == 1) {
            ...
            if (sender instanceof Player player && !player.getUniqueId().equals(target.getUniqueId())) {
                boolean isStaff = PermissionUtils.has(player, STAFF_PERMISSION)
                        || PermissionUtils.has(player, FLY_STAFF_PERMISSION)
                        || PermissionUtils.has(player, "ultimatedonutsmp2.command.flyspeed.others");
                if (!isStaff) {
                    player.sendMessage(ColorUtils.toComponent(
                            plugin.getConfigManager().getMessageOrDefault("STAFF.NO_PERMISSION_OTHERS", "&cYou do not have permission.")
                    ));
