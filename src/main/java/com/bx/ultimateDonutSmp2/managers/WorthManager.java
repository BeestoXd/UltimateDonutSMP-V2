...
public class WorthManager {
    ...
    static final String DEFAULT_WORTH_LORE_FORMAT = "&#00FC00$ &f{price}";
    private static final Pattern LEGACY_WORTH_LABEL = Pattern.compile("(?i)^worth:\\s*.+$");
    private static final Pattern LEGACY_TILDE_WORTH_LINE = Pattern.compile("^~\\s*\\$\\s*.+$");
    ...
    public WorthManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    ...
    public List<SellWorthEntry> resolveSellWorthEntries(ItemStack item) {
        return resolveSellWorthEntries(item, true);
    }

    /**
     * @param expandContainers false leaves shulkers and other containers packed, so an unpriced
     *                         full container is skipped instead of being sold for its contents.
     *                         Players choose this with the "Include Shulker on Sell" setting.
     */
    public List<SellWorthEntry> resolveSellWorthEntries(ItemStack item, boolean expandContainers) {
        if (item == null || item.getType().isAir()) {
            return List.of();
        }

        List<SellWorthEntry> entries = new ArrayList<>();
        collectSellWorthEntries(item, 1, 0, expandContainers, new HashSet<>(), entries);
    ...
    private boolean isWorthLoreLine(String line, Pattern worthLorePattern) {
        String plain = ColorUtils.strip(line).trim();
        if (containsBrokenWorthPlaceholder(plain)) {
            return true;
        }
        if (LEGACY_WORTH_LABEL.matcher(plain).matches() || LEGACY_TILDE_WORTH_LINE.matcher(plain).matches()) {
            return true;
        }
    ...
    private String getWorthLoreFormat() {
        ...
        FileConfiguration worthConfig = plugin.getConfigManager().getWorth();
        return normalizeWorthLoreFormat(firstNonBlank(
                mainConfig == null ? null : mainConfig.getString("WORTH-LORE.FORMAT"),
                worthConfig == null ? null : worthConfig.getString("WORTH-LORE.FORMAT"),
                worthConfig == null ? null : worthConfig.getString("DISPLAY.FORMAT"),
                worthConfig == null ? null : worthConfig.getString("FORMAT")
        ));
    }
    ...
    private String normalizeWorthLoreFormat(String format) {
        if (format == null || format.isBlank() || isLegacyDefaultWorthLoreFormat(format)) {
            return DEFAULT_WORTH_LORE_FORMAT;
        }

        return format;
    }

    private boolean isLegacyDefaultWorthLoreFormat(String format) {
        String normalized = format.trim();
        return normalized.equals("&7Worth: &a$%price%")
                || normalized.equals("&7Worth: &a$${price}")
                || normalized.equals("&7Worth: &a${price}")
                || normalized.equalsIgnoreCase("&7worth: &a{price_formatted}");
    }
