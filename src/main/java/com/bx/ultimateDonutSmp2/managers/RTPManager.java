...
public class RTPManager {
    ...
    private static final long DEFAULT_MIN_SEARCH_DISPLAY_TICKS = 0L;
    private static final int DEFAULT_SEARCH_ATTEMPTS_PER_TICK = 1;
    private static final long DEFAULT_FOUND_DISPLAY_TICKS = 0L;
    ...
    private static final String MIN_PLAYER_DISTANCE_SETTING = "SETTINGS.MIN-PLAYER-DISTANCE";
    private static final int DEFAULT_MIN_PLAYER_DISTANCE = 2000;
    private static final String COOLDOWN_PERMISSION_PREFIX = "ultimatedonutsmp2.rtp.cooldown.";
    ...
    private static final String LOCATION_CACHE_BACKGROUND_WARM_UP_SETTING = "SETTINGS.LOCATION-CACHE.BACKGROUND-WARM-UP";
    ...
    private static final class SearchProgress {
        ...
        private SearchProgress(String worldName, SearchSettings settings) {
            this(worldName, settings, null);
        }

        private SearchProgress(String worldName, SearchSettings settings, Location origin) {
            this.worldName = worldName;
            this.settings = settings;
            this.hasOrigin = origin != null;
            this.originX = origin == null ? 0D : origin.getX();
            this.originZ = origin == null ? 0D : origin.getZ();
        }
    }
    ...
    public RTPManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
        reload();
    }
    ...
    public int getPlayerPriority(Player player) {
        ...
        for (org.bukkit.permissions.PermissionAttachmentInfo pai : player.getEffectivePermissions()) {
            ...
            if (perm != null && pai.getValue() && perm.toLowerCase(Locale.ROOT).startsWith("ultimatedonutsmp2.rtp.priority.")) {
                String sub = perm.substring("ultimatedonutsmp2.rtp.priority.".length());
                try {
    ...
    public boolean isBackgroundWarmUpEnabled() {
        if (!isPreCacheEnabled()) {
            return false;
        }
        return plugin.getConfigManager().getRtp().getBoolean(LOCATION_CACHE_BACKGROUND_WARM_UP_SETTING, false);
    }
     ...
     *
     * <p>RTP is instant by default. Raise this only if the searching action bar should stay
     * on screen after a spot is already known. 0 teleports as soon as a spot is known.</p>
     */
     ...
     * cache. RTP is instant by default, so 0 queues the teleport immediately instead of
     * waiting for surrounding chunks.</p>
     */
    ...
    /** Starts filling cached RTP spots for menu destinations so a click can teleport immediately. */
    public void prefetchMenuDestinations() {
        if (!isPreCacheReady() || getPreCacheSize() <= 0) {
            return;
        }
        for (RTPDestination destination : menuDestinations) {
            if (destination != null && destination.enabled() && refillPreCache(destination.worldName())) {
                return;
            }
        }
    }
    ...
    private boolean startPreCacheSearch(String worldKey, SearchSettings settings) {
        ...
        future.whenComplete((location, throwable) -> {
            ...
            if (!isUsefulCacheLocation(settings, location)) {
                return;
            }
    ...
    /**
     * Off unless the admin turned {@code LOCATION-CACHE.GENERATE-CHUNKS} on. Loading already
     * generated chunks from disk still pulls them into the ticking world, which is what dropped
     * TPS the moment the first player joined and the warm-up started.
     */
    static boolean shouldLoadDiskChunksForBackgroundWarmUp(boolean cacheGenerateChunks) {
        return cacheGenerateChunks;
    }
    ...
    private int getPreCacheParallelAttempts() {
        return preCacheParallelAttempts(isPreCacheChunkGenerationEnabled(), getSearchAttemptsPerTick());
    }

    static int preCacheParallelAttempts(boolean generateChunks, int playerFacingAttempts) {
        if (!generateChunks) {
            return 1;
        }
        return Math.max(1, Math.min(PRE_CACHE_PARALLEL_ATTEMPTS, playerFacingAttempts));
    }
    ...
    private Location pollPreCachedLocation(String worldName) {
        return pollPreCachedLocation(worldName, null);
    }

    private Location pollPreCachedLocation(String worldName, Location origin) {
        ...
        java.util.Queue<CachedLocation> tooClose = new ConcurrentLinkedQueue<>();
        CachedLocation entry;
        while ((entry = cached.poll()) != null) {
            if (!isCachedLocationUsable(entry)) {
                continue;
            }
            if (!isFarEnoughFromOrigin(origin, entry.location(), getMinPlayerDistance(getWorldSearchSettings(worldName)))) {
                tooClose.add(entry);
                continue;
            }
            cached.addAll(tooClose);
    ...
    private void stopSearch(UUID playerId, boolean clearActionBar) {
        stopSearch(playerId, clearActionBar, true);
    }

    private void stopSearch(UUID playerId, boolean clearActionBar, boolean advanceQueue) {
    ...
    public boolean queueCommandTeleport(Player player, String selector) {
        ...
        if (player != null && player.getWorld() != null && isDeniedWorld(player.getWorld().getName())) {
    ...
    private void runDirectSearchStep(DirectSearchState state, CompletableFuture<Location> future) {
        ...
        if (state.backgroundWarmUp && !shouldLoadDiskChunksForBackgroundWarmUp(state.allowChunkGeneration)) {
            FileConfiguration rtp = plugin.getConfigManager().getRtp();
            if (rtp == null || !rtp.getBoolean(LOAD_GENERATED_CHUNKS_SETTING, true)) {
                sampleLoadedChunkForWarmUp(state, future, world, settings);
                return;
            }
        }
        ...
        // Spawn-loaded chunks sit next to the player. Caching them, or using them as a
        // fallback, is what made RTP look instant and then dump someone 300 blocks away.
        if (!generateFallback
                && !state.backgroundWarmUp
                && shouldUseLoadedChunkFallback(chunkSamplesUsed)) {
    ...
    private void sampleLoadedChunkForWarmUp(
            DirectSearchState state,
            CompletableFuture<Location> future,
            World world,
            SearchSettings settings
    ) {
        state.chunkSamplesUsed.getAndIncrement();
        plugin.getSpigotScheduler().runRegion(world, settings.centerX() >> 4, settings.centerZ() >> 4, () -> {
            int[] loadedChunk = nextLoadedChunkSample(world);
            if (loadedChunk == null) {
                future.complete(null);
            ...
            plugin.getSpigotScheduler().runRegion(world, loadedChunk[0], loadedChunk[1], () -> {
                Location found = null;
                try {
                    LocationAttempt attempt = tryLoadedChunkLocationAttempt(settings, loadedChunk[0], loadedChunk[1]);
                    if (attempt.countedAttempt()) {
                        state.attemptsUsed.incrementAndGet();
                    }
                    found = attempt.location();
    ...
    private void continueDirectSearch(DirectSearchState state, CompletableFuture<Location> future, Location found) {
        if (found != null && state.backgroundWarmUp && !isUsefulCacheLocation(state.settings, found)) {
            found = null;
        }
        if (found != null) {
    ...
    /**
     * A cached spot has to be far enough from the world centre that a player standing at spawn
     * can actually use it. Nearby loaded chunks pass the safe-stand check and then get rejected
     * the moment someone clicks RTP, which is the wait after the button.
     */
    private boolean isUsefulCacheLocation(SearchSettings settings, Location found) {
        if (found == null || settings == null) {
            return false;
        }
        return isFarEnoughFromOrigin(
                settings.centerX(),
                settings.centerZ(),
                found.getX(),
                found.getZ(),
                getMinPlayerDistance(settings)
        );
    }
    ...
    private boolean queueTeleport(Player player, String worldName) {
        ...
        if (reqHours > 0.0) {
            com.bx.ultimateDonutSmp2.models.PlayerData data = plugin.getPlayerDataManager().get(player);
    ...
    private void startSearch(Player player, String worldName, SearchSettings settings) {
        stopSearch(player.getUniqueId(), false, false);
        ...
        Location origin = sameWorldOrigin(player, worldName);
        Location preCached = pollPreCachedLocation(worldName, origin);
        if (preCached != null) {
            finishSearch(player, worldName, preCached);
            return;
        }

        SearchProgress progress = new SearchProgress(worldName, settings, origin);
        activeSearches.put(player.getUniqueId(), progress);

        // Folia treats a 0-tick timer delay as 1 tick, which is a visible pause after the RTP
        // button. Kick the first attempts off on this tick with the Player we already have;
        // Bukkit.getPlayer can be null in tests and during some join/quit edges.
        if (plugin.getSpigotScheduler() != null) {
            tickSearch(player);
            if (activeSearches.containsKey(player.getUniqueId())) {
                UUID playerId = player.getUniqueId();
    ...
    private void tickSearch(Player player) {
        ...
        if (!player.isOnline() || progress == null) {
            clearSearch(playerId);
            return;
        }
    ...
    private void beginAsyncLocationAttempt(UUID playerId, SearchProgress progress) {
        ...
        if (!generateFallback && shouldUseLoadedChunkFallback(progress)) {
            ...
            plugin.getSpigotScheduler().runRegion(world, progress.settings.centerX() >> 4, progress.settings.centerZ() >> 4, () -> {
                int[] loadedChunk = nextLoadedChunkSample(world, progress);
                if (loadedChunk == null) {
        ...
        int[] sample = nextRandomSample(progress.settings, progress);
        int x = sample[0];
        int z = sample[1];
        int chunkX = x >> 4;
        int chunkZ = z >> 4;
    ...
    private void completeAsyncLocationAttempt(UUID playerId, SearchProgress progress, LocationAttempt attempt, Throwable throwable) {
        ...
        if (found != null && !isFarEnoughFromOrigin(progress, found)) {
            found = null;
        }
        if (found != null) {
    ...
    private void finishSearch(Player player, String worldName, Location found) {
        ...
        BukkitTask resultTask = plugin.getSpigotScheduler().runEntityTimer(player, () -> {
            ...
            if (shownTicks[0] >= foundDisplayTicks) {
                queuePreparedRtpTeleport(player, found, playerId, resultTaskRef, queuedTeleport);
                refillPreCache(worldName);
            }
    ...
    private LocationAttempt tryFindSafeLocationAttempt(SearchSettings settings) {
        ...
        int[] sample = nextRandomSample(settings, null);
        int x = sample[0];
        int z = sample[1];
    ...
    private int[] nextRandomSample(SearchSettings settings) {
        return nextRandomSample(settings, null);
    }

    private int[] nextRandomSample(SearchSettings settings, SearchProgress progress) {
        int minRadius = Math.max(0, settings.minRadius());
        int maxRadius = Math.max(minRadius, settings.maxRadius());
        double originX;
        double originZ;
        int minPlayerDistance;
        if (progress != null && progress.hasOrigin) {
            originX = progress.originX;
            originZ = progress.originZ;
            minPlayerDistance = getMinPlayerDistance(progress);
        } else {
            originX = settings.centerX();
            originZ = settings.centerZ();
            minPlayerDistance = getMinPlayerDistance(settings);
        }
        minRadius = innerSampleRadius(
                settings.centerX(),
                settings.centerZ(),
                minRadius,
                maxRadius,
        ...
        int[] lastSample = null;
        int retries = minPlayerDistance > 0 ? 48 : 1;
        for (int attempt = 0; attempt < retries; attempt++) {
            ...
            lastSample = new int[] {x, z};
            if (isFarEnoughFromOrigin(originX, originZ, x, z, minPlayerDistance)) {
                return lastSample;
            }
        }
        return lastSample;
    ...
    /**
     * Raises the inner ring when the origin is still near the world centre. Sampling 500–2000
     * around spawn can never satisfy a 2000-block player distance, so those attempts only delay
     * the teleport after the RTP button.
     */
    static int innerSampleRadius(
            int centerX,
            int centerZ,
            int minRadius,
            int maxRadius,
        ...
        int inner = Math.max(0, minRadius);
        int outer = Math.max(inner, maxRadius);
        if (minPlayerDistance <= 0 || inner >= outer) {
            return inner;
        }
        double originFromCenter = Math.hypot(originX - centerX, originZ - centerZ);
        if (originFromCenter < minPlayerDistance) {
            return Math.min(outer, Math.max(inner, minPlayerDistance));
        }
        return inner;
    }
    ...
    int[] nextLoadedChunkSample(World world) {
        return nextLoadedChunkSample(world, null);
    }

    int[] nextLoadedChunkSample(World world, SearchProgress progress) {
        if (world == null) {
            return null;
        }

        Chunk[] loadedChunks = world.getLoadedChunks();
        if (loadedChunks == null || loadedChunks.length == 0) {
            return null;
        }

        int minPlayerDistance = getMinPlayerDistance(progress);
        int attempts = minPlayerDistance > 0 ? Math.min(32, loadedChunks.length) : 1;
        int[] lastSample = null;
        for (int attempt = 0; attempt < attempts; attempt++) {
            Chunk chunk = loadedChunks[ThreadLocalRandom.current().nextInt(loadedChunks.length)];
            lastSample = new int[] {chunk.getX(), chunk.getZ()};
            int x = (chunk.getX() << 4) + 8;
            int z = (chunk.getZ() << 4) + 8;
            if (isFarEnoughFromOrigin(progress, x, z, minPlayerDistance)) {
                return lastSample;
            }
        }
        return null;
    ...
    private Location sameWorldOrigin(Player player, String worldName) {
    ...
    int getMinPlayerDistance(SearchSettings settings) {
        FileConfiguration rtp = plugin.getConfigManager() == null ? null : plugin.getConfigManager().getRtp();
        int fallback = rtp == null
                ? DEFAULT_MIN_PLAYER_DISTANCE
                : Math.max(0, rtp.getInt(MIN_PLAYER_DISTANCE_SETTING, DEFAULT_MIN_PLAYER_DISTANCE));
        if (settings == null) {
            return fallback;
        }
        ConfigurationSection worldSettings = getWorldSettingsSection(settings.worldName());
        int configured = worldSettings == null
                ? fallback
                : Math.max(0, worldSettings.getInt("MIN-PLAYER-DISTANCE", fallback));
        int maxRadius = Math.max(0, settings.maxRadius());
        if (maxRadius <= 0) {
            return configured;
        }
        return Math.min(configured, maxRadius);
    }

    private int getMinPlayerDistance(SearchProgress progress) {
        if (progress == null || !progress.hasOrigin) {
            return 0;
        }
        return getMinPlayerDistance(progress.settings);
    }

    static boolean isFarEnoughFromOrigin(double originX, double originZ, double x, double z, int minDistance) {
        if (minDistance <= 0) {
            return true;
        }
        double dx = x - originX;
    ...
    private boolean isFarEnoughFromOrigin(Location origin, Location target, int minDistance) {
        if (origin == null || target == null || minDistance <= 0) {
            return true;
        }
        return isFarEnoughFromOrigin(origin.getX(), origin.getZ(), target.getX(), target.getZ(), minDistance);
    }

    private boolean isFarEnoughFromOrigin(SearchProgress progress, Location target) {
        if (progress == null || !progress.hasOrigin || target == null) {
            return true;
        }
        return isFarEnoughFromOrigin(
                progress.originX,
                ...
                target.getX(),
                target.getZ(),
                getMinPlayerDistance(progress)
        );
    }

    private boolean isFarEnoughFromOrigin(SearchProgress progress, double x, double z, int minDistance) {
        if (progress == null || !progress.hasOrigin || minDistance <= 0) {
            return true;
        }
        return isFarEnoughFromOrigin(progress.originX, progress.originZ, x, z, minDistance);
    }

    private boolean shouldUseLoadedChunkFallback(SearchProgress progress) {
        if (getMinPlayerDistance(progress) > 0) {
            return false;
        }
    ...
    public boolean hasExplicitWorldSettings(String worldName) {
        if (worldName == null || worldName.isBlank()) {
            return false;
        }
        if (plugin == null || plugin.getConfigManager() == null || plugin.getConfigManager().getRtp() == null) {
        ...
        if (worlds.getConfigurationSection(worldName) != null) {
            return true;
        }
    ...
    public boolean isLobbyWorld(String worldName) {
        ...
        String name = worldName.trim().toLowerCase(Locale.ROOT);
        if (plugin != null && plugin.getConfigManager() != null && plugin.getConfigManager().getRtp() != null) {
            List<String> configuredLobbies = plugin.getConfigManager().getRtp().getStringList("LOBBY-WORLDS");
            for (String lobby : configuredLobbies) {
                if (lobby != null && lobby.trim().equalsIgnoreCase(name)) {
                    return true;
                }
            }
        }
        return name.equals("lobby")
                || name.equals("hub")
                || name.equals("spawn");
    }
    ...
    private String normalizeConfiguredWorldName(String worldName) {
        ...
        return switch (ascii.toLowerCase(Locale.ROOT)) {
            ...
            default -> {
                if (isLobbyWorld(trimmed) && !hasExplicitWorldSettings(trimmed)) {
                    yield getLoadedNormalWorldName();
                }
                yield trimmed;
            }
        };
