...
public class ShopManager {
    ...
    private static final class PendingSale {
        ...
        /** Mirrors the seller's "Include Shulker on Sell" setting for the whole sale. */
        private boolean expandContainers = true;
    }
    ...
    public ShopManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    ...
    public Map<Integer, QuickBuyEntry> getQuickBuyEntries(UUID playerId) {
        Map<Integer, QuickBuyEntry> entries = getPreference(playerId).quickBuyEntries();
        if (entries.isEmpty()) {
            Map<Integer, QuickBuyEntry> defaults = getDefaultQuickBuyEntries();
            if (!defaults.isEmpty()) {
                defaults.forEach((slot, entry) -> setQuickBuyEntry(playerId, entry));
                return defaults;
            }
        }
        return entries;
    ...
    public Map<Integer, QuickBuyEntry> getDefaultQuickBuyEntries() {
        org.bukkit.configuration.ConfigurationSection section =
                plugin.getConfigManager().getShop().getConfigurationSection("QUICK-BUY.DEFAULT-ENTRIES");
        if (section == null) {
            return Map.of();
        }
        Map<Integer, QuickBuyEntry> defaults = new HashMap<>();
        for (String key : section.getKeys(false)) {
            try {
                int slot = Integer.parseInt(key);
                String matStr = section.getString(key + ".MATERIAL", "AIR");
                Material mat = Material.getMaterial(matStr.toUpperCase(Locale.ROOT));
                int amount = Math.max(1, section.getInt(key + ".AMOUNT", 1));
                if (mat != null && !mat.isAir()) {
                    defaults.put(slot, new QuickBuyEntry(slot, mat, amount, null, true));
                }
    ...
    public void setQuickBuyEntry(UUID playerId, QuickBuyEntry entry) {
        if (entry == null) {
            return;
        }
        preferenceCache.compute(playerId, (ignored, current) -> {
            ShopPreference base = current == null
                    ? new ShopPreference(playerId, Set.of())
                    : current;
            return base.withQuickBuy(entry.slot(), entry);
        });
        preferenceRepository.setQuickBuy(playerId, entry).exceptionally(throwable -> {
            plugin.getLogger().log(Level.WARNING, "Failed to save Quick Buy entry for " + playerId, throwable);
            return null;
        });
    }

    public void removeQuickBuyEntry(UUID playerId, int slot) {
        preferenceCache.compute(playerId, (ignored, current) -> {
            if (current == null) {
                return new ShopPreference(playerId, Set.of());
            }
            return current.withoutQuickBuy(slot);
        });
        preferenceRepository.removeQuickBuy(playerId, slot).exceptionally(throwable -> {
            plugin.getLogger().log(Level.WARNING, "Failed to remove Quick Buy entry for " + playerId, throwable);
            return null;
        });
    }

    public QuickBuyQuote resolveQuickBuyQuote(Player buyer, QuickBuyEntry entry) {
    ...
    public QuickBuyQuote resolveQuickBuyQuote(Player buyer, Material material, int amount, Map<org.bukkit.enchantments.Enchantment, Integer> requiredEnchants) {
    ...
    public QuickBuyQuote resolveQuickBuyQuote(Material material, int amount) {
    ...
    public CompletableFuture<QuickBuyExecutionResult> executeQuickBuy(Player buyer, QuickBuyEntry entry) {
        ...
        QuickBuyQuote quote = resolveQuickBuyQuote(buyer, entry);
        if (quote.outOfStock()) {
    ...
    public List<ShopItem> loadMenuItems(String menuSection) {
        ...
        for (String key : sourceSection.getKeys(false)) {
            ...
            AmethystToolType amethystType = AmethystToolType.fromString(itemSec.getString("AMETHYST-TOOL"));
            long amethystDuration = itemSec.getLong("AMETHYST-DURATION", -1L);
        ...
        if (item.isAmethystToolReward()) {
            ...
            for (int index = 0; index < quantity; index++) {
                ItemStack reward = plugin.getAmethystToolsManager().createTool(
                        ...
                        AmethystToolAppearance.fromShop(item.displayName(), item.lore(), item.enchantments())
                );
                if (reward == null) {
                ...
                applyConfiguredItemExtras(reward, item);
                rewards.add(reward);
            }
    ...
    private ItemStack createPurchasedItem(ShopItem item, int amount) {
        ...
        applyConfiguredItemExtras(stack, item);
        if (shouldKeepShopIdentity(item)) {
            stack = ItemUtils.withDisplay(
                    stack,
                    item.displayName(),
                    AmethystToolAppearance.stripPriceLines(item.lore())
            );
            applyPurchasedDisplayFlags(stack, item);
        }
        return stack;
    }

    private boolean shouldKeepShopIdentity(ShopItem item) {
        if (item == null) {
            return false;
        }
        ...
        ConfigurationSection section = plugin.getConfigManager().getShop()
                .getConfigurationSection(item.menuSection() + "." + item.key());
        return section != null && section.getBoolean("KEEP-DISPLAY", false);
    }

    private void applyConfiguredItemExtras(ItemStack stack, ShopItem item) {
        if (stack == null || item == null || plugin.getConfigManager() == null || plugin.getConfigManager().getShop() == null) {
            return;
        }
        ConfigurationSection section = plugin.getConfigManager().getShop()
                .getConfigurationSection(item.menuSection() + "." + item.key());
    ...
    private void applyPurchasedDisplayFlags(ItemStack stack, ShopItem item) {
        if (stack == null || item == null || plugin.getConfigManager() == null || plugin.getConfigManager().getShop() == null) {
            return;
        }
        ConfigurationSection section = plugin.getConfigManager().getShop()
                .getConfigurationSection(item.menuSection() + "." + item.key());
        boolean potion = stack.getType() == Material.POTION
                || stack.getType() == Material.SPLASH_POTION
                || stack.getType() == Material.LINGERING_POTION;
        ItemUtils.hideMenuNoise(stack, !potion);
        if (section != null && section.getBoolean("HIDE-ENCHANTS", false)) {
            ItemUtils.hideEnchantments(stack);
        }
    }

    /**
     * The exact item stored as ITEM-DATA, or null when the entry is a plain MATERIAL one.
     * Menus use this so the icon matches what the buyer actually receives.
     */
    ...
    private PendingSale createPendingSale(Player player) {
        ...
        sale.expandContainers = data == null || data.isSellShulkersEnabled();
        return sale;
    }

    /**
     * What the given inventory range is worth to this player right now, without selling
     * anything. The sell menu puts this on its confirm button.
     */
    public double previewSellWorth(Player player, Inventory inventory, int startInclusive, int endExclusive) {
        if (player == null || inventory == null) {
            return 0;
        }

        PendingSale preview = createPendingSale(player);
        int firstSlot = Math.max(0, startInclusive);
        int lastSlot = Math.min(inventory.getSize(), Math.max(firstSlot, endExclusive));
        double total = 0;
        for (int slot = firstSlot; slot < lastSlot; slot++) {
            ...
            double payout = collectSellWorthEntries(item, preview);
            if (payout > 0) {
                total += payout;
            }
        }
    ...
    private double collectSellWorthEntries(ItemStack item, PendingSale sale) {
        List<WorthManager.SellWorthEntry> entries =
                plugin.getWorthManager().resolveSellWorthEntries(item, sale.expandContainers);
        if (entries.isEmpty()) {
            return 0;
        }
    ...
    public void sendPurchaseActionBar(Player player, String rawItemName, int amount, double price) {
        sendPurchaseActionBar(player, rawItemName, amount, price, Currency.MONEY);
    }

    public void sendPurchaseActionBar(Player player, String rawItemName, int amount, double price, Currency currency) {
        if (player == null) {
            return;
        }

        String template;
        if (currency == Currency.SHARD) {
            template = plugin.getConfigManager().getShop().getString(
                    "QUICK-BUY.ACTION-BAR-SHARDS",
                    "&fYou bought {amount} {item} for &5{price} Shards"
            );
        } else {
            template = plugin.getConfigManager().getShop().getString("QUICK-BUY.ACTION-BAR");
            if (template == null || template.isBlank()) {
                template = plugin.getConfigManager().getMessageOrDefault(
                        "QUICK_BUY.ACTION_BAR",
                        "&fYou bought {amount} {item} for &a$ &f{price}"
                );
            }
        }

        if (template.equalsIgnoreCase("none") || template.equalsIgnoreCase("disabled") || template.isBlank()) {
            return;
        }

        String cleanItemName = rawItemName != null ? ColorUtils.strip(rawItemName).trim() : "Item";
        String pluralItem = formatPurchasedItemName(cleanItemName, amount);
        String nicePrice = NumberUtils.formatNice(price);
        String formattedPrice = currency == Currency.SHARD
                ? (long) price + " Shards"
                : plugin.getCurrencyManager().formatMoney(price);
        String rawPrice = NumberUtils.format(price);

        String msg = template
                .replace("{amount}", String.valueOf(amount))
                .replace("{quantity}", String.valueOf(amount))
                .replace("{item}", pluralItem)
                .replace("{item-name}", pluralItem)
                .replace("{item_name}", pluralItem)
                .replace("{item_singular}", cleanItemName)
                .replace("{price}", nicePrice)
                .replace("{price_nice}", nicePrice)
                .replace("{price_formatted}", formattedPrice)
                .replace("{price_raw}", rawPrice);

        PlayerSettingUtils.sendActionBar(plugin, player, msg);
    }

    public static String formatPurchasedItemName(String baseName, int amount) {
        if (baseName == null || baseName.isBlank()) {
            return "";
        }
        String clean = ColorUtils.strip(baseName).trim();
        if (amount <= 1) {
            return clean;
        }
        String lower = clean.toLowerCase(Locale.ROOT);
        if (lower.equals("obsidian") || lower.equals("glowstone") || lower.equals("elytra")
                || lower.equals("tnt") || lower.equals("glass") || lower.endsWith(" glass")
                || lower.endsWith(" wool") || lower.endsWith(" terracotta") || lower.endsWith(" concrete")
                || lower.endsWith(" sand") || lower.endsWith(" gravel") || lower.endsWith(" ice")
                || lower.endsWith(" air")) {
            return clean;
        }
        if (lower.contains(" of ")) {
            int ofIndex = clean.indexOf(" of ");
            String first = clean.substring(0, ofIndex);
            String rest = clean.substring(ofIndex);
            if (!first.toLowerCase(Locale.ROOT).endsWith("s")) {
                first = first + "s";
            }
            return first + rest;
        }
        if (lower.endsWith("s") || lower.endsWith("sh") || lower.endsWith("ch") || lower.endsWith("x") || lower.endsWith("z")) {
            return clean.endsWith("s") ? clean : clean + "es";
        }
        return clean + "s";
    }
