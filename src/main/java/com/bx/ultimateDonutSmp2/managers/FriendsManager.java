...
public class FriendsManager {
    ...
    public FriendsManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }

    public void applyPreloaded(UUID playerUuid, List<FollowEntry> following, List<FollowEntry> followers) {
        if (playerUuid == null) {
            return;
        }

        Map<UUID, FollowEntry> followingEntries = new ConcurrentHashMap<>();
        if (following != null) {
            for (FollowEntry entry : following) {
                if (entry != null && entry.followedUuid() != null) {
                    followingEntries.put(entry.followedUuid(), entry);
                }
            }
        }
        followingMap.put(playerUuid, followingEntries);

        Map<UUID, FollowEntry> followerEntries = new ConcurrentHashMap<>();
        if (followers != null) {
            for (FollowEntry entry : followers) {
                if (entry != null && entry.followerUuid() != null) {
                    followerEntries.put(entry.followerUuid(), entry);
                }
            }
        }
        followersMap.put(playerUuid, followerEntries);
    }
    ...
    public boolean followPlayer(Player follower, UUID followedUuid, String followedName) {
        ...
        notifyFollowed(follower, followedUuid);
        return true;
    }

    private void notifyFollowed(Player follower, UUID followedUuid) {
        Player onlineTarget = Bukkit.getPlayer(followedUuid);
        if (onlineTarget == null || !onlineTarget.isOnline()) {
            return;
        }
        PlayerData targetData = plugin.getPlayerDataManager() == null
                ? null
                : plugin.getPlayerDataManager().get(onlineTarget);
        if (targetData != null && !targetData.isFollowAlertsEnabled()) {
            return;
        }
        String senderName = plugin.getHideManager() != null
                ? plugin.getHideManager().publicName(follower)
                : follower.getName();
        onlineTarget.sendMessage(ColorUtils.toComponent(plugin.getConfigManager().getMessageOrDefault(
                "FRIENDS.FOLLOW_ALERT_RECEIVED",
                "&e&lFollow &7» &b" + senderName + " &7is now following you."
        ), onlineTarget));
    }
    ...
    public boolean isTransactionMessageBlocked(UUID actorUuid, UUID recipientUuid) {
        if (plugin.getPlayerDataManager() != null) {
            PlayerData actorData = plugin.getPlayerDataManager().get(actorUuid);
            if (actorData != null && actorData.isPrivateTransactionsEnabled()) {
                return true;
            }
        }
    ...
    public void handleJoin(Player player) {
        ...
        if (!followingMap.containsKey(playerUuid) || !followersMap.containsKey(playerUuid)) {
