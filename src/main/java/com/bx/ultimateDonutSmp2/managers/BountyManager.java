...
public class BountyManager {
    ...
    public BountyManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    private void preloadBountySkins() {
        if (plugin.getPlayerSkinManager() == null) {
            return;
        }
        for (Bounty bounty : bounties.values()) {
            UUID targetUuid = bounty.getTargetUuid();
            String name = getDisplayName(targetUuid);
            TablistManager.SkinTexture texture = plugin.getPlayerSkinManager().resolveSkinTexture(targetUuid, name);
            if (texture == null) {
                plugin.getPlayerSkinManager().resolveSkinTextureAsync(targetUuid, name, null);
            }
        }
    }
    ...
    public String getDisplayName(UUID playerUuid) {
        ...
        if (plugin.getHideManager() != null) {
            return plugin.getHideManager().publicName(playerUuid);
        }
