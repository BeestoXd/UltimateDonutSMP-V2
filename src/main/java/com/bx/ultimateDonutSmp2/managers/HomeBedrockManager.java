...
public final class HomeBedrockManager {
    ...
    public HomeBedrockManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }

    private String homesTitle() {
        if (plugin.getConfigManager() == null || plugin.getConfigManager().getDialog() == null) {
            return "Homes";
        }
        return plugin.getConfigManager().getDialog().getString("HOMES_DIALOG.TITLE", "Homes");
    }
