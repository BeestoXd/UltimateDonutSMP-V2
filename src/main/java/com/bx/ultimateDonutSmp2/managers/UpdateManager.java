...
public class UpdateManager {
    ...
    private static final String VERSION_URL = "https://raw.githubusercontent.com/BeestoXd/UltimateDonutSMP-V2/main/version.txt";

    private final UltimateDonutSmp2 plugin;
    private String latestVersion = null;
    private boolean updateAvailable = false;

    public UpdateManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
                    ...
                    if (version != null) {
                        ...
                        if (!version.isEmpty()) {
                            ...
                            if (this.updateAvailable) {
                                Bukkit.getConsoleSender().sendMessage(ColorUtils.colorize("&c[UltimateDonutSmp2] A new update is available: " + latestVersion + " (Current version: " + currentVersion + ")"));
                                Bukkit.getConsoleSender().sendMessage(ColorUtils.colorize("&c[UltimateDonutSmp2] Please download the latest version from: https://github.com/BeestoXd/UltimateDonutSMP-V2"));
                            } else {
                                plugin.getLogger().info("You are running the latest version of UltimateDonutSmp2 (" + currentVersion + ").");
                            }
                        }
                    }
                }
