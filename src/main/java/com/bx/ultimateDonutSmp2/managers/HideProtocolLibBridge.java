...
final class HideProtocolLibBridge implements HidePacketBridge {
    ...
    private PacketListener nametagMovementListener;
    private BukkitTask nametagRideTask;

    HideProtocolLibBridge(UltimateDonutSmp2 plugin, HideManager hideManager) {
        this.plugin = plugin;
    ...
    private void startNametagRideTask() {
        ...
        registerNametagMovementListener();
    ...
    /**
     * Entity-move packets fire for every living thing the client can see. Listening to them while
     * nobody has an alias nametag out costs a ProtocolLib trip on the hottest packet path in the
     * game, so they are only hooked while a display exists.
     */
    private void registerNametagMovementListener() {
        if (nametagMovementListener != null) {
            return;
        }
        nametagMovementListener = new PacketAdapter(
                plugin,
                ListenerPriority.HIGHEST,
            ...
            public void onPacketSending(PacketEvent event) {
                ...
                suppressNametagMovement(event);
            }
        };
        protocolManager.addPacketListener(nametagMovementListener);
    }

    private void unregisterNametagMovementListener() {
        if (nametagMovementListener == null) {
            return;
        }
        protocolManager.removePacketListener(nametagMovementListener);
        nametagMovementListener = null;
    }
    ...
    private void stopNametagRideTask() {
        unregisterNametagMovementListener();
