...
public class DuelWorldManager {
    ...
    public DuelWorldManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    private boolean shouldDeferWorldCreation() {
        OptimizationManager optimization = plugin.getOptimizationManager();
        return optimization != null && optimization.getLoadState() == OptimizationManager.LoadState.CRITICAL;
    }
    ...
    private boolean hasFlatPoolWork() {
        return shouldPrepareGeneratedPool(!requestedFlatBiomes.isEmpty(), false);
    }
    ...
    private void startFlatPreparation(Biome biome) {
        if (biome == null || shouldDeferWorldCreation()) {
            return;
        }
    ...
    private boolean hasVanillaPoolWork() {
        return shouldPrepareGeneratedPool(!requestedVanillaBiomes.isEmpty(), activeVanillaPreparation != null);
    }
    ...
    private int getVanillaPoolPrepareIntervalTicks() {
        return Math.max(10, config().getInt(VANILLA_POOL_PATH + ".PREPARE_INTERVAL_TICKS", 20));
    }
    ...
    /**
     * Generated duel worlds are created only after someone actually queues or a preparation
     * already started. Filling an idle pool on startup calls {@code createWorld()} on the
     * main thread and stalls keepalives even with nobody online.
     */
    static boolean shouldPrepareGeneratedPool(boolean hasRequestedArenas, boolean hasActivePreparation) {
        return hasActivePreparation || hasRequestedArenas;
    }
