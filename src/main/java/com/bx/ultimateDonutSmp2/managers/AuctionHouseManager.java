...
import com.bx.ultimateDonutSmp2.models.AuctionBrowseEngine;
...
import com.bx.ultimateDonutSmp2.models.PlayerAuctionSession;
import com.bx.ultimateDonutSmp2.models.PlayerPreference;
import com.bx.ultimateDonutSmp2.storage.AuctionHouseRepository;
...
public final class AuctionHouseManager {
    ...
    public AuctionHouseManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    ...
    public static String formatBrowseTitle(String template, int page) {
        String base = template != null && !template.isBlank() ? template : "Auction (Page {page})";
        return base.replace("{page}", String.valueOf(page));
    }
    ...
    public String getBrowseTitle(int page) {
        return formatBrowseTitle(config().getString("GUI.BROWSE.TITLE", "Auction (Page {page})"), page);
    }
    ...
    public List<AuctionListing> getPlayerTransactions(UUID playerId, String search) {
        long ninetyDaysAgo = System.currentTimeMillis() - java.util.concurrent.TimeUnit.DAYS.toMillis(90);
        return listingCache.get().stream()
                .filter(listing -> listing.status() == AuctionListing.Status.SOLD)
                .filter(listing -> (listing.soldAt() > 0 ? listing.soldAt() : listing.createdAt()) >= ninetyDaysAgo)
                .filter(listing -> listing.sellerUuid().equals(playerId) || (listing.buyerUuid() != null && listing.buyerUuid().equals(playerId)))
                .filter(listing -> {
                    if (search == null || search.isBlank()) return true;
                    String s = search.toLowerCase(java.util.Locale.ROOT);
                    if (listing.sellerName() != null && listing.sellerName().toLowerCase(java.util.Locale.ROOT).contains(s)) return true;
                    if (listing.item() != null) {
                        String itemName = describeItem(listing.item());
                        if (itemName != null && itemName.toLowerCase(java.util.Locale.ROOT).contains(s)) return true;
                        if (listing.item().getType().name().toLowerCase(java.util.Locale.ROOT).contains(s)) return true;
                    }
                    return false;
                })
                .sorted(Comparator.comparingLong((AuctionListing l) -> l.soldAt() > 0 ? l.soldAt() : l.createdAt()).reversed()
                        .thenComparing(Comparator.comparingLong(AuctionListing::id).reversed()))
                .toList();
    }
    ...
    public double getPlayerTotalSpent(UUID playerId) {
        return listingCache.get().stream()
                .filter(l -> l.status() == AuctionListing.Status.SOLD && l.buyerUuid() != null && l.buyerUuid().equals(playerId))
                .mapToDouble(AuctionListing::price)
                .sum();
    }

    public double getPlayerTotalMade(UUID playerId) {
        return listingCache.get().stream()
                .filter(l -> l.status() == AuctionListing.Status.SOLD && l.sellerUuid().equals(playerId))
                .mapToDouble(AuctionListing::price)
                .sum();
    }
    ...
    public CompletableFuture<CancelListingResult> cancelListing(Player owner, long listingId) {
        ...
        if (!canFitItem(owner, listing.item())) {
            return CompletableFuture.completedFuture(
                    new CancelListingResult(false, CancelFailureReason.INVENTORY_FULL, listing)
            );
        }
        ...
        return repository.cancelListing(listingId, owner.getUniqueId(), System.currentTimeMillis())
                ...
                .thenCompose(cancelled -> {
                    ...
                    AuctionClaim claim = getUnclaimedClaims(owner.getUniqueId()).stream()
                            .filter(c -> c.sourceListingId() == cancelledListing.id() && c.itemClaim())
                            .findFirst()
                            .orElse(null);
                    if (claim != null) {
                        return collectClaim(owner, claim.id()).thenApply(claimResult -> {
                            plugin.getPlayerLogsManager().log(
                                    owner.getUniqueId(),
                                    owner.getName(),
                                    "Auctions",
                        ...
                        plugin.getSpigotScheduler().runEntity(owner, () -> restoreEscrow(owner, cancelledListing.item()));
                        plugin.getPlayerLogsManager().log(
                                owner.getUniqueId(),
                                owner.getName(),
                                "Auctions",
                        ...
                        return CompletableFuture.completedFuture(
                                new CancelListingResult(true, null, cancelledListing)
                        );
                    }
                })
                .exceptionally(throwable -> {
    ...
    public int getMaxActiveListings(Player player) {
        ...
        resolved = Math.max(resolved, PermissionUtils.resolveHighestExactNumberedPermission(
                player,
                "ultimatedonutsmp2.auctionhouse.limit.",
                MAX_PERMISSION_VALUE
        ));
        resolved = Math.max(resolved, PermissionUtils.resolveHighestExactNumberedPermission(
                player,
    ...
    public void restoreEscrow(Player player, ItemStack item) {
        if (item == null || item.getType().isAir()) {
            return;
        }
        if (!player.isOnline()) {
    ...
    private void notifySellerOfSale(AuctionListing listing, Player buyer) {
        ...
        String message = plugin.getConfigManager().getMessageOrDefault(
                ...
                "&#00FC88{buyer} &7bought your {item} for &#00FC88${price}",
        ...
        String actionBar = plugin.getConfigManager().getMessageOrDefault(
                "AUCTION_HOUSE.ITEM_SOLD_ACTION_BAR",
                "&#00FC88{buyer} &7bought your {item} for &#00FC88${price}",
                "{buyer}", plugin.getHideManager().publicName(buyer),
                "{item}", describeItem(listing.item()),
                "{price}", NumberUtils.format(listing.price()),
        ...
        PlayerSettingUtils.sendActionBar(plugin, seller, actionBar);
        AuctionHouseSounds.play(seller, plugin, AuctionHouseSounds.BOUGHT_YOUR_ITEM);
    }
    ...
    private boolean hasPermission(Player player, String action) {
        if (PermissionUtils.has(player, "ultimatedonutsmp2.admin.auctionhouse")) {
            return true;
        }
        if (PermissionUtils.has(player, "ultimatedonutsmp2.auctionhouse." + action)
                || PermissionUtils.has(player, "donutauction." + action)) {
            return true;
        }
        if ("buy".equalsIgnoreCase(action)
                && (PermissionUtils.has(player, "ultimatedonutsmp2.auctionhouse.use")
                || PermissionUtils.has(player, "donutauction.use"))) {
            return !isExplicitlyDenied(player, "ultimatedonutsmp2.auctionhouse.buy")
