...
import com.bx.ultimateDonutSmp2.utils.OptimizationLoadPolicy;
...
public class OptimizationManager {
    ...
    private static final long DEFAULT_STARTUP_GRACE_TICKS = 1200L;

    private BukkitTask monitorTask;
    private boolean enabled;
    private boolean logStateChanges;
    private long monitorIntervalTicks;
    private long startupGraceTicks;
    ...
    public OptimizationManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
        reload();
    }
    ...
    public void start() {
        ...
        long initialDelay = Math.max(monitorIntervalTicks, remainingGraceTicks());
    ...
    public void reload() {
        ...
        startupGraceTicks = Math.max(0L, config.getLong(
                CONFIG_PATH + ".STARTUP-GRACE-TICKS", DEFAULT_STARTUP_GRACE_TICKS));
    ...
    private void sampleServerLoad() {
        ...
        lastMspt = readMspt();
        lastSampleMillis = System.currentTimeMillis();
        if (startedAtMillis > 0L && OptimizationLoadPolicy.inStartupGrace(
                lastSampleMillis - startedAtMillis,
                startupGraceTicks * MILLIS_PER_TICK)) {
            return;
        }
    ...
    private long remainingGraceTicks() {
        if (startupGraceTicks <= 0L || startedAtMillis <= 0L) {
            return 0L;
        }
        long elapsedTicks = Math.max(0L, System.currentTimeMillis() - startedAtMillis) / MILLIS_PER_TICK;
        return Math.max(0L, startupGraceTicks - elapsedTicks);
    }

    private LoadState classify(double tps, double mspt) {
        return switch (OptimizationLoadPolicy.classify(tps, mspt, criticalTps, warnTps, warnMspt)) {
            case CRITICAL -> LoadState.CRITICAL;
            case WARN -> LoadState.WARN;
