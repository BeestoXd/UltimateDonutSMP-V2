...
public final class OrdersBedrockManager {
    ...
    public OrdersBedrockManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean openCollect(Player player) {
        ...
        form.validResultHandler(response -> schedule(player, () -> {
            ...
            if (button == 0) {
                ...
                var result = plugin.getOrdersManager().claimBatch(player, 0L, false);
                playCollectResult(player, result, false);
                openCollect(player);
            } else if (button == 2) {
                var result = plugin.getOrdersManager().claimBatch(player, 0L, true);
                playCollectResult(player, result, true);
                openCollect(player);
            } else {
                int index = button - 3;
                if (index >= 0 && index < claims.size()) {
                    var claim = claims.get(index);
                    var result = plugin.getOrdersManager().claim(player, claim.id());
                    plugin.getOrdersManager().playSound(player, result.success()
                            ? (claim.refundClaim() ? "ORDERS.REFUND" : "ORDERS.COLLECT")
                            : "ORDERS.FAIL");
                    openCollect(player);
                }
            }
    ...
    private void openSearch(Player player, String currentQuery) {
        ...
        form.validResultHandler(response ->
                schedule(player, () -> {
                    ...
                    openMain(player, response.asInput(0));
                }));
    ...
    private void openNewOrder(Player player, OrderCatalogEntry entry) {
        ...
        form.validResultHandler(response -> schedule(player, () -> {
            try {
                ...
                sendResult(player, result.success(), "ORDERS.CREATE",
                ...
                sendResult(player, false, "ORDERS.FAIL", "ORDERS.BEDROCK.INVALID_NUMBER",
    ...
    private void openDelivery(Player player, Order order) {
        ...
        form.validResultHandler(response -> schedule(player, () -> {
            try {
                ...
                sendResult(player, result.success(), "ORDERS.DELIVER",
                ...
                sendResult(player, false, "ORDERS.FAIL", "ORDERS.BEDROCK.INVALID_NUMBER", "enter a valid amount.");
                openDelivery(player, order);
            }
        }));
    ...
    private void openAfterCollect(Player player, long orderId) {
        if (plugin.getOrdersManager().hasUnclaimedItemClaims(player.getUniqueId(), orderId)) {
            openOwnerActions(player, plugin.getOrdersManager().getOrder(orderId));
            return;
        }
        openMyOrders(player);
    }
    ...
    private void openCancelConfirmation(Player player, Order order) {
        ModalForm.Builder form = ModalForm.builder()
                ...
                .content(text("ORDERS.BEDROCK.CANCEL.CONTENT",
                        "Cancel order #{id}? Remaining money will be refunded.",
                        "{id}", String.valueOf(order.id())))
        ...
        form.validResultHandler(response -> schedule(player, () -> {
            if (response.clickedFirst()) {
                ...
                sendResult(player, result.success(), "ORDERS.CANCEL",
    ...
    private void sendResult(Player player, boolean success, String successSound, String path, String fallback, String... placeholders) {
        player.sendMessage(ColorUtils.toComponent((success ? "&a" : "&c") + text(path, fallback, placeholders)));
        plugin.getOrdersManager().playSound(player, success ? successSound : "ORDERS.FAIL");
    }

    private void playCollectResult(Player player, OrderBatchClaimResult result, boolean dropItems) {
        if (result == null || (result.itemClaims() <= 0 && result.refundClaims() <= 0)) {
            plugin.getOrdersManager().playSound(player, "ORDERS.FAIL");
            return;
        }
        if (dropItems && result.itemClaims() > 0) {
            plugin.getOrdersManager().playSound(player, "ORDERS.DROP");
            return;
        }
        if (result.itemClaims() <= 0 && result.refundClaims() > 0) {
            plugin.getOrdersManager().playSound(player, "ORDERS.REFUND");
            return;
        }
        plugin.getOrdersManager().playSound(player, "ORDERS.COLLECT");
    }
