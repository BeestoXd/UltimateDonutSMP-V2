...
import com.bx.ultimateDonutSmp2.utils.PacketBelowNameRenderer;
...
public class MoneyNametagManager {
    ...
    public MoneyNametagManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    ...
    public void updateAll() {
        ...
        Map<UUID, String> texts = snapshotTexts();
    ...
    private Map<UUID, String> snapshotTexts() {
        String format = config().getString("MONEY-NAMETAGS.FORMAT", "&a$ &f{balance}");
        boolean shortFormat = config().getBoolean("MONEY-NAMETAGS.SHORT-FORMAT", true);
        Map<UUID, String> texts = new HashMap<>();
        for (Player target : Bukkit.getOnlinePlayers()) {
            if (shouldDisplayFor(target)) {
                texts.put(target.getUniqueId(), ColorUtils.colorize(
                        render(format, plugin.getEconomyManager().getBalance(target), shortFormat),
                        target));
            }
        }
        return texts;
    ...
    private void push(Player viewer, Map<UUID, String> precomputedTexts) {
        ...
        for (Player target : Bukkit.getOnlinePlayers()) {
            ...
            String text = precomputedTexts != null
                    ? precomputedTexts.get(target.getUniqueId())
                    : ColorUtils.colorize(currentText(target), target);
            if (text == null) {
                known.remove(target.getUniqueId());
                continue;
            }
