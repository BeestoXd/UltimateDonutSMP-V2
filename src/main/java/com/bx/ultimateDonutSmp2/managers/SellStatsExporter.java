...
public class SellStatsExporter {
    ...
    public SellStatsExporter(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
    }
    ...
    public static synchronized void startEmbeddedHttpServer(UltimateDonutSmp2 plugin) {
                ...
                server.createContext("/stats", exchange -> {
                    try {
                        ...
                        if ("/stats/reset".equals(path) || "/stats/reset/".equals(path)) {
                            ...
                            boolean isLocalhost = remote != null && (remote.getAddress().isLoopbackAddress() || "127.0.0.1".equals(remote.getAddress().getHostAddress()));
                            if (!isLocalhost) {
                                byte[] response = "{\"error\":\"Forbidden: reset only allowed from localhost\"}".getBytes(StandardCharsets.UTF_8);
    ...
    public static synchronized void restartEmbeddedHttpServer(UltimateDonutSmp2 plugin) {
    ...
    public void exportHtml(CommandSender sender) {
        plugin.getSpigotScheduler().runAsync(() -> {
            try {
                ...
                if (sender != null) {
                    sender.sendMessage(ColorUtils.toComponent("&a&l[Sell Stats Site] &fHTML web dashboard generated at: &eplugins/UltimateDonutSmp2/sell-stats.html"));
                }
    ...
    public void uploadToWebPaste(CommandSender sender) {
        ...
        if (sender instanceof Player player && player.isOnline()) {
            ...
            linkMsg.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(ColorUtils.toComponent("&7Click to open &e" + localWebUrl + "\n&7File: plugins/UltimateDonutSmp2/sell-stats.html"))));
            player.spigot().sendMessage(linkMsg);
        } else if (sender != null) {
            sender.sendMessage(ColorUtils.toComponent("&a&l[Sell Stats Web] &fLive web site: &e" + localWebUrl + " &7(or open plugins/UltimateDonutSmp2/sell-stats.html)"));
        }
    }
    ...
    private void sendWebServerOfflineNotice(CommandSender sender) {
        ...
        sender.sendMessage(ColorUtils.toComponent("&7An offline copy is still saved to &eplugins/UltimateDonutSmp2/sell-stats.html&7."));
    }
    ...
    public String generateDashboardHtml() {
        ...
        html.append("<title>Shop Analytics — UltimateDonutSMP2</title>\n");
