...
import com.bx.ultimateDonutSmp2.hooks.SimpleVoiceChatHook;
import com.bx.ultimateDonutSmp2.menus.VoiceChatConsentMenu;
...
import com.bx.ultimateDonutSmp2.models.VoiceChatConsent;
...
public class VoiceChatConsentManager {
    ...
    public VoiceChatConsentManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
        refreshSettings();
    }
    ...
    public void refreshVoiceMute(UUID uuid, String name) {
        ...
        applyVoiceMute(uuid, record);
    }

    public void applyVoiceMute(UUID uuid, PunishmentRecord record) {
        if (uuid == null) {
            return;
        }
        if (record == null) {
    ...
    public void handleJoin(Player player) {
        handleJoin(player, null, false);
    }

    public void handleJoin(Player player, PunishmentRecord preloadedVoiceMute, boolean voiceMutePreloaded) {
        if (player == null) {
            return;
        }
        VoiceChatConsent consent = getConsent(player);
        consentCache.put(player.getUniqueId(), consent);
        if (voiceMutePreloaded) {
            applyVoiceMute(player.getUniqueId(), preloadedVoiceMute);
        } else {
