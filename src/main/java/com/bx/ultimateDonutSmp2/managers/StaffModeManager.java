...
import com.bx.ultimateDonutSmp2.dialogs.ChatClickMessages;
import com.bx.ultimateDonutSmp2.menus.FrozenPlayersMenu;
import com.bx.ultimateDonutSmp2.menus.StaffListMenu;
import com.bx.ultimateDonutSmp2.models.FreezeState;
import com.bx.ultimateDonutSmp2.models.StaffModeState;
import com.bx.ultimateDonutSmp2.staff.StaffCustomItem;
import com.bx.ultimateDonutSmp2.staff.StaffInventorySnapshot;
import com.bx.ultimateDonutSmp2.staff.StaffModeSession;
...
public class StaffModeManager {
    ...
    public StaffModeManager(UltimateDonutSmp2 plugin) {
        this.plugin = plugin;
        reloadInternal(true);
    ...
    public String getStaffPermission() {
        return getConfig().getString("STAFF-MODE.STAFF-PERMISSION", "ULTIMATEDONUTSMP2.STAFF.MODE");
    }

    public String getAdminPermission() {
        return getConfig().getString("STAFF-MODE.ADMIN-PERMISSION", "ULTIMATEDONUTSMP2.ADMIN.STAFFMODE");
    }

    public String getVanishPermission() {
        return getConfig().getString("STAFF-MODE.VANISH-PERMISSION", "ULTIMATEDONUTSMP2.STAFF.MODE.VANISH");
    }

    public String getBetterViewPermission() {
        return getConfig().getString("STAFF-MODE.BETTER-VIEW-PERMISSION", "ULTIMATEDONUTSMP2.STAFF.MODE.BETTERVIEW");
    }

    public String getStaffListPermission() {
        return getConfig().getString("STAFF-MODE.STAFF-LIST-PERMISSION", "ULTIMATEDONUTSMP2.STAFF.MODE.STAFFLIST");
    }
    ...
    public String getOthersPermission() {
        return getConfig().getString("STAFF-MODE.OTHERS-PERMISSION", "ULTIMATEDONUTSMP2.STAFF.MODE.OTHERS");
    }
    ...
    public Player teleportToRandomPlayer(Player viewer) {
        ...
        String message = formatLegacyMessage(
                getRandomTeleportMessage("SUCCESS", "&8[&eRTP&8] &7you were teleported to &e%player%"),
                target.getName(),
                null,
                null,
                target.getName(),
        ...
        plugin.getSpigotScheduler().teleport(viewer, target.getLocation()).thenAccept(success -> {
            ...
            plugin.getSpigotScheduler().runEntity(viewer, () -> {
                if (viewer.isOnline()) {
                    sendClickableRandomTeleportMessage(viewer, message, targetId);
                ...
                plugin.getSpigotScheduler().runEntity(target, () -> {
                    if (target.isOnline()) {
                        target.sendMessage(ColorUtils.toComponent(
                                formatLegacyMessage(
                                        getRandomTeleportMessage("NOTIFY", "&8[&eRTP&8] &e%player% &7appeared near you!"),
                                        viewerName,
                                        null,
                                        null,
                                        viewerName,
    ...
    /**
     * The {@code [RTP]} chat line teleports again on click. A {@code RUN_COMMAND} click would
     * open Minecraft's command-confirm dialog, which this flow no longer uses.
     */
    private void sendClickableRandomTeleportMessage(Player viewer, String message, UUID targetId) {
        boolean sent = ChatClickMessages.sendInstantAction(
                plugin,
                viewer,
                message,
                "&eClick to teleport",
                clicker -> teleportToClickedPlayer(clicker, targetId)
        );
        if (!sent) {
    ...
    private void teleportToClickedPlayer(Player viewer, UUID targetId) {
        if (targetId == null) {
            return;
        }
        ...
        plugin.getSpigotScheduler().teleport(viewer, target.getLocation());
    }
    ...
    public boolean isStaffMember(Player player) {
        ...
        return PermissionUtils.has(player, getStaffPermission())
                ...
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.invsee")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.profileviewer")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.view")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.create")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.remove")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.ban")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.unban")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.mute")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.unmute")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.blacklist")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.unblacklist")
                || PermissionUtils.has(player, "ultimatedonutsmp2.staff.punishments.delete");
    }
