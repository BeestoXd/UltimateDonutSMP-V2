...
public class ItemUtils {
    ...
    public static List<String> cleanLoreList(List<String> rawLore) {
        if (rawLore == null) return null;
        List<String> cleaned = new ArrayList<>(rawLore.size());
    ...
    public static List<String> readLore(ConfigurationSection config, String path, List<String> fallback) {
        if (config == null || path == null) {
            return fallback != null ? new ArrayList<>(fallback) : new ArrayList<>();
        }
        if (config.isList(path)) {
            List<String> list = config.getStringList(path);
            if (!list.isEmpty()) {
                return cleanLoreList(list);
            }
        }
        String single = config.getString(path);
        if (single != null && !single.isBlank()) {
            return cleanLoreList(List.of(single));
        }
        return fallback != null ? new ArrayList<>(fallback) : new ArrayList<>();
    }
    ...
    public static ItemStack createItem(Material material, String displayName, List<String> lore) {
        ...
        if (lore != null && !lore.isEmpty()) {
            meta.setLore(ColorUtils.colorizeList(cleanLoreList(lore)));
        }
    ...
    /**
     * Adds lore to a copy without replacing the vanilla item name.
     */
    ...
    public static ItemStack hideMenuNoise(ItemStack item, boolean hideAdditionalTooltip) {
        if (item == null) {
            return null;
        }
        ItemMeta meta = item.getItemMeta();
    ...
    public static ItemStack applyConfiguredPotion(ItemStack item, ConfigurationSection section) {
        if (item == null || section == null || !(item.getItemMeta() instanceof PotionMeta meta)) {
            return item;
        }

        List<String> effects = section.getStringList("POTION-EFFECTS");
        for (String entry : effects) {
            PotionEffect effect = parsePotionEffect(entry);
            if (effect != null) {
                meta.addCustomEffect(effect, true);
            }
        }

        Color color = parseHexColor(section.getString("POTION-COLOR"));
        if (color != null) {
            meta.setColor(color);
        }
    ...
    public static PotionEffect parsePotionEffect(String entry) {
        ...
        PotionEffectType type = potionEffectType(parts[0].trim());
        if (type == null) {
            return null;
        }
        try {
            int amplifier = Integer.parseInt(parts[1].trim());
            int durationTicks = Integer.parseInt(parts[2].trim());
    ...
    public static Material resolveMobHeadMaterial(String mobTypeKey) {
        if (mobTypeKey == null || mobTypeKey.isBlank()) {
            return Material.SKELETON_SKULL;
        }
        String key = mobTypeKey.trim().toUpperCase(Locale.ROOT);
        return switch (key) {
            // Skeleton variants
            case "SKELETON", "STRAY", "BOGGED", "SKELETON_HORSE" -> Material.SKELETON_SKULL;

            // Wither & Nether dark boss/monsters
            case "WITHER_SKELETON", "WITHER", "BLAZE", "WARDEN" -> Material.WITHER_SKELETON_SKULL;

            // Zombie variants & Humanoid monsters
            case "ZOMBIE", "HUSK", "DROWNED", "ZOMBIE_VILLAGER", "ZOMBIE_HORSE", "GIANT" -> Material.ZOMBIE_HEAD;

            // Creeper & explosive / slime / arthropod mobs
            case "CREEPER", "SLIME", "MAGMA_CUBE", "SPIDER", "CAVE_SPIDER", "BEE", "SILVERFISH" -> Material.CREEPER_HEAD;

            // Piglin variants & Pig / Swine / Animal mobs
            case "PIGLIN", "PIGLIN_BRUTE", "ZOMBIFIED_PIGLIN", "PIG", "HOGLIN", "ZOGLIN", "STRIDER" -> Material.PIGLIN_HEAD;

            // Dragon & Ender / Flying / Boss mobs
            case "DRAGON", "ENDER_DRAGON", "ENDERMAN", "ENDERMITE", "SHULKER", "PHANTOM", "GHAST", "BREEZE" -> Material.DRAGON_HEAD;

            // Villagers & Illagers
            case "VILLAGER", "WANDERING_TRADER", "WITCH", "PILLAGER", "VINDICATOR", "EVOKER", "ILLUSIONER", "RAVAGER", "IRON_GOLEM" -> Material.ZOMBIE_HEAD;

            // Common Animals / Peaceful Mobs
            case "COW", "MOOSHROOM", "MUSHROOM_COW", "HORSE", "DONKEY", "MULE", "LLAMA", "TRADER_LLAMA", "CAMEL", "PANDA", "POLAR_BEAR" -> Material.ZOMBIE_HEAD;
            case "SHEEP", "CHICKEN", "RABBIT", "WOLF", "FOX", "CAT", "OCELOT", "PARROT", "ARMADILLO", "SNOW_GOLEM", "SNOWMAN" -> Material.SKELETON_SKULL;
            case "SQUID", "GLOW_SQUID", "DOLPHIN", "TURTLE", "AXOLOTL", "COD", "SALMON", "PUFFERFISH", "TROPICAL_FISH", "TADPOLE", "FROG", "ALLAY", "BAT", "VEX" -> Material.ZOMBIE_HEAD;
            case "GUARDIAN", "ELDER_GUARDIAN", "SNIFFER" -> Material.DRAGON_HEAD;

            // Fallback for any other monster or mob:
            default -> {
                if (key.contains("WITHER")) yield Material.WITHER_SKELETON_SKULL;
                if (key.contains("SKELETON") || key.contains("BONE")) yield Material.SKELETON_SKULL;
                if (key.contains("CREEPER") || key.contains("SPIDER") || key.contains("SLIME")) yield Material.CREEPER_HEAD;
                if (key.contains("PIG") || key.contains("HOG")) yield Material.PIGLIN_HEAD;
                if (key.contains("DRAGON") || key.contains("ENDER")) yield Material.DRAGON_HEAD;
                yield Material.ZOMBIE_HEAD;
            }
        };
    }

    public static Material resolveExactVanillaMobHead(String mobTypeKey) {
        if (mobTypeKey == null || mobTypeKey.isBlank()) {
            return null;
        }
        String key = mobTypeKey.trim().toUpperCase(Locale.ROOT);
        return switch (key) {
            ...
            case "DRAGON", "ENDER_DRAGON" -> Material.DRAGON_HEAD;
            default -> null;
        };
    }

    public static ItemStack createMobHead(String mobTypeKey, String displayName, List<String> lore) {
        return createMobHead(mobTypeKey, null, displayName, lore);
    }

    public static ItemStack createMobHead(String mobTypeKey, String textureValue, String displayName, List<String> lore) {
        Material vanillaHead = resolveExactVanillaMobHead(mobTypeKey);
        if (vanillaHead != null) {
            return createItem(vanillaHead, displayName, lore);
        }

        if (textureValue != null && !textureValue.isBlank()) {
            return createHeadFromSkinUrl(textureValue, displayName, lore);
        }

        String skinUrl = getMobSkinUrl(mobTypeKey);
        if (skinUrl != null && !skinUrl.isBlank()) {
            return createHeadFromSkinUrl(skinUrl, displayName, lore);
        }

        Material fallbackMat = resolveMobHeadMaterial(mobTypeKey);
        if (fallbackMat == Material.PLAYER_HEAD) {
            fallbackMat = Material.SKELETON_SKULL;
        }
        return createItem(fallbackMat, displayName, lore);
    }
    ...
    public static String getMobSkinUrl(String mobTypeKey) {
        ...
        return switch (key) {
            case "COW" -> "https://textures.minecraft.net/texture/61527b63294bebcb2a21d2b0e4f3da50c2c65d6c5354185881e675c123c331c5";
