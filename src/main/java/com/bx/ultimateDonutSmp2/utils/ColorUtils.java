...
import com.bx.ultimateDonutSmp2.managers.LanguageManager;
...
public class ColorUtils {
    ...
    public static String colorize(String text) {
        ...
        return applyColors(applyPlaceholders(text, null));
    }
    ...
    public static String colorize(String text, Player player) {
        ...
        return applyColors(applyPlaceholders(text, player));
    }
    ...
    /**
     * PlaceholderAPI only understands {@code %tokens%}. Scoreboard and tablist lines hit this
     * thousands of times a second, so a missing percent sign skips the expansion entirely.
     */
    private static String applyPlaceholders(String text, OfflinePlayer player) {
        if (!hasPAPI || text.indexOf('%') < 0) {
            return text;
        }
        try {
            return player instanceof Player online
                    ? me.clip.placeholderapi.PlaceholderAPI.setPlaceholders(online, text)
    ...
    private static final Pattern ROMAN_NUMERAL_PATTERN = Pattern.compile("^(?i)(M{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{1,3})|[IVXLCDM]+)$");

    public static boolean isRomanNumeral(String text) {
        if (text == null || text.isBlank()) {
            return false;
        }
        return ROMAN_NUMERAL_PATTERN.matcher(text.trim()).matches();
    }
