...
One permanent map. Elo, an LT/HT ladder, kits, and an optional schematic reset.
Ships **off**. Command: `/pvp`. File: [pvp.yml](Config-pvp.yml).

This is not [instanced FFA](Duels-and-FFA). FFA copies a map and throws it away. Here the
rating carries over; the blocks stay.
...
Give yourself `ultimatedonutsmp2.admin.pvp` and work through this in the world the arena lives in.
...
/pvp kit permission warrior ultimatedonutsmp2.pvp.kit.warrior
...
what you want when running the same fixture repeatedly. Both need `ultimatedonutsmp2.admin.pvp`.
...
The bundled [Discord bot example](https://github.com/BeestoXd/UltimateDonutSMP-V2/tree/main/discord-bot-example)
...
the ender chest, the block itself. Holders of `ultimatedonutsmp2.admin.pvp` are exempt from all of
it.
